...
class CSP(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if self.regularize_cov:
            cov_matrices = np.array([(1 / (n_samples - 1)) * np.dot(X_t, X_t.T) +
                                     (self.rho/n_samples)*np.eye(n_channels)
                                     for X_t in X])
        else:
        ...
        for c1 in range(n_classes):
            for c2 in range(c1 + 1, n_classes):
                ...
                assert np.allclose(cov_avg[c1] @ U - cov_avg[c2] @ U @
                                   np.diag(eig_vals), np.zeros((n_channels, n_channels)))
                ...
                self.w = eig_vecs if len(self.w) == 0 \
                    else np.hstack([self.w, eig_vecs])
                # Elements {i, i+m} go in pairs
                self.pairs_idx.append(chosen_idxs)

        self.n_csp = 2*m*np.sum(range(n_classes))
        assert self.w.shape == (n_channels, self.n_csp), 'Got w of shape {} instead of {}.'.format(
            self.w.shape, [n_channels, self.n_csp])
    ...
    def transform(self, X):
        ...
        X_transformed = np.array([self.w.T @ X[trial_idx]
        ...
        variances = np.array([X_transformed[trial_idx] @ X_transformed[trial_idx].T
                              for trial_idx in range(n_trials)])
        assert variances.shape == (n_trials, n_csp, n_csp)

        # Compute normalized log-variance features
        feats = np.array([np.log10(np.diag(variances[trial_idx]) /
                                   np.trace(variances[trial_idx]))
                          for trial_idx in range(n_trials)])
