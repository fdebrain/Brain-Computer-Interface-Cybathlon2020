...
class Formatter:
    ...
    def extracting(self, raw):
        ...
        if self.preprocess:
            raw = mne_apply(lambda a: filtering(a, f_low=0.5, f_high=100, fs=raw.info["sfreq"], f_order=5), raw)
    ...
    def resampling(self, raw, events, new_fs):
        ...
        if new_fs == raw.info["sfreq"]:
            ...
            return deepcopy(raw)
        print("Resampling from {:f} to {:f} Hz.".format(raw.info["sfreq"], new_fs))

        data = raw.get_data().T

        new_data = resampy.resample(data, raw.info["sfreq"], new_fs, axis=0, filter="kaiser_fast").T
    ...
    def labelling(self, events, signal_len, file_idx):
        ...
        if self.mode == "train":                
            ...
            data = scipy.io.loadmat(self.root + 'test/labels/{}.mat'.format(file_idx))
            true_labels = np.array(data['classlabel']) + min(self.labels_idx) - 1
            print('True labels set: ', np.unique(true_labels))
            labels = self.labelling_test(events, signal_len, true_labels, self.unknown_idx)

        # Sanity check
        print("Counting labels after labelling")
        for label in self.labels_idx:
            print("\t {}: {:.2f}".format(label, len(np.where(labels==label)[0]) / (self.fs*(self.pre + self.post))))
    ...
    def saving(self, X, y=None, file_name='data1'):
        ...
        if self.mode not in os.listdir(self.save_path + self.save_folder):
            os.mkdir(path=self.save_path + self.save_folder + '/{}'.format(self.mode))
        ...
        if self.save_as_trial:
            for trial_idx in range(X.shape[0]):
                np.savez_compressed(self.save_path + '{}/{}/{}'.format(self.save_folder, self.mode, trial_idx), X=X[trial_idx], y=y[trial_idx])
        else:        
            print("Saving data of shape: ", X.shape)
            np.savez_compressed(self.save_path + '{}/{}/{}'.format(self.save_folder, self.mode, file_name), X=X, y=y)
...
class FormatterNPZ(Formatter):    
    def __init__(self, root, save_path, labels_idx, ch_list, remove_ch, pre, post, fs, mode, save, save_folder):
        super().__init__(root, save_path, labels_idx, ch_list, remove_ch, pre, post, mode, save, save_folder, fs=fs)
    ...
    def run(self):
        ...
        for pilot_idx, filepath in enumerate(filepaths):
            ...
            eeg = raw['s']
            events = np.stack([raw['epos'].reshape(-1),raw['edur'].reshape(-1), raw['etyp'].reshape(-1)],axis=1)
...
class FormatterGDF(Formatter):
    def __init__(self, root, save_path, labels_idx, ch_list, remove_ch, pre, post, mode, save, save_folder, resample=False, preprocess=False, unknown_idx=4, fs=None, save_as_trial=False, multisession=False, multithread=False):
        super().__init__(root, save_path, labels_idx, ch_list, remove_ch, pre, post, mode, save, save_folder, resample, preprocess, unknown_idx, fs, save_as_trial)
    ...
    def run(self):
        ...
        if self.multithread:
            ...
            results = Parallel(n_jobs=num_cores, verbose=100)(delayed(self.sub_run)(pilot, filepaths_pilot) for pilot,filepaths_pilot in enumerate(filepaths_pilots))
        else:
    ...
    def sub_run(self, pilot_idx, filepaths_pilot):
        ...
        for session_idx in range(len(filepaths_pilot)):
            ...
            raw = mne.io.read_raw_gdf(filepaths_pilot[session_idx], preload=True)
        ...
        # Save as .npy file
        if self.save:
            self.saving(pilot_eeg, pilot_labels, '{}{}'.format(self.mode, pilot_idx+1))
        print("")
...
class FormatterVHDR(Formatter):
    def __init__(self, root, save_path, pilot_idx, session_idx, labels_idx, ch_list, remove_ch, pre, post, mode, save, save_folder, resample=False, preprocess=False, unknown_idx=4, control=False, save_as_trial=False):
        super().__init__(root, save_path, labels_idx, ch_list, remove_ch, pre, post, mode, save, save_folder, resample, preprocess, unknown_idx, fs=None, save_as_trial=save_as_trial)
    ...
    def run(self):
        ...
        if self.control:
            filepaths_pilot = glob.glob(self.root + 'Control_{}/Session_{}/vhdr/*.vhdr'.format(self.pilot_idx, self.session_idx))
            self.save_path = self.root + 'Control_{}/Session_{}/'.format(self.pilot_idx, self.session_idx)
        else:
            filepaths_pilot = glob.glob(self.root + 'Pilot_{}/Session_{}/vhdr/{}*.vhdr'.format(self.pilot_idx, self.session_idx, 'test/' if self.mode=='test/' else ''))
            self.save_path = self.root + 'Pilot_{}/Session_{}/'.format(self.pilot_idx, self.session_idx)
        ...
        for sub_session_idx in range(len(filepaths_pilot)):
            print("Start formatting " + filepaths_pilot[sub_session_idx] + "...")
            ...
            event_ids = dict(zip(['left', 'right', 'both', 'rest'], self.labels_idx))
            epochs = mne.Epochs(raw, events, event_ids, -self.pre, self.post, baseline=None, preload=True)
...
class FormatterMAT(Formatter):
    ...
    def run(self):
        ...
        for pilot_idx in range(len(filepaths_all)):
            ...
            all_class_names = ["".join(chr(c) for c in data[obj_ref]) for obj_ref in class_name_set]
            print('Classes: ', all_class_names)
            
            # Extract eeg
            ch_names = [ 'ch{}'.format(c) for c in range(1,134)]
            eeg = []
            eeg = np.array([ data[ch_name][:].squeeze() for ch_name in ch_names ])
            print(eeg.shape)
                
            # Extract events
            events_c = data['mrk']['event']['desc'][:].reshape(-1,1).astype(int)
