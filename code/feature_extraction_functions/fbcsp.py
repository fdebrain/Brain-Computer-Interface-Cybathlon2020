...
class FBCSP(TransformerMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        if self.k>0:
            self.feature_selection = SelectKBest(mutual_info_classif, k=self.k)
    ...
    def transform(self, X):
        ...
        if self.k>0:
            ...
            for idx in select_idxs:
                sub_idx = idx%(2*m) # feature index inside 2*m block of features
                comp_idx = idx + (m if sub_idx<m else -m) # corresponding paired index

                pair_feats = np.hstack([feats[:, idx].reshape(-1,1), feats[:, comp_idx].reshape(-1,1)])
                selected_feats = pair_feats if len(selected_feats)==0 else np.hstack([selected_feats, pair_feats])
                #print(idx, comp_idx, selected_feats.shape)
        else:
