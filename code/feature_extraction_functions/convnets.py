...
def EEGNet(nb_classes=4, n_channels=22, n_samples=int(250*7.5), 
    ...
    block1       = DepthwiseConv2D((n_channels, 1), 
                                   use_bias = False, 
                                   depth_multiplier = D, # Defines nbr of output filters (D*F1//undersample)
                                   padding = 'valid',
...
def DeepConvNet(nb_classes=4, n_channels = 22, n_samples = 1875,
    ...
    block1       = Conv2D(25, (n_channels, 1), kernel_constraint=max_norm(2.), kernel_regularizer=l2(l2_reg))(block1)
    block1       = BatchNormalization(axis=1, epsilon=1e-05, momentum=0.1)(block1)
    block1       = Activation('elu')(block1)
    block1       = MaxPooling2D(pool_size=(1, 3), strides=(1, 3))(block1)
    block1       = Dropout(dropoutRate)(block1)
  
    block2       = Conv2D(50, (1, 10//undersample), kernel_constraint = max_norm(2.), kernel_regularizer=l2(l2_reg))(block1)
    block2       = BatchNormalization(axis=1, epsilon=1e-05, momentum=0.1)(block2)
    block2       = Activation('elu')(block2)
    block2       = MaxPooling2D(pool_size=(1, 3), strides=(1, 3))(block2)
    block2       = Dropout(dropoutRate)(block2)
    
    block3       = Conv2D(100, (1, 10//undersample), kernel_constraint = max_norm(2.), kernel_regularizer=l2(l2_reg))(block2)
    block3       = BatchNormalization(axis=1, epsilon=1e-05, momentum=0.1)(block3)
    block3       = Activation('elu')(block3)
    block3       = MaxPooling2D(pool_size=(1, 3), strides=(1, 3))(block3)
    block3       = Dropout(dropoutRate)(block3)
    
    block4       = Conv2D(200, (1, 10//undersample), kernel_constraint = max_norm(2.), kernel_regularizer=l2(l2_reg))(block3)
...
def ShallowConvNet(nb_classes=4, n_channels = 22, n_samples = 1125, dropoutRate = 0.5, l2_reg=0.01, undersample=1):
    ...
    block1       = Conv2D(40, (1, 25//undersample), 
                          ...
                          kernel_regularizer=l2(l2_reg), # New
    ...
    block1       = BatchNormalization(axis=1, epsilon=1e-05, momentum=0.1)(block1)
    block1       = Activation('elu')(block1) # New
    
    # Similar to power band feature extraction (log-variance)
    block1       = Lambda(lambda x: K.square(x))(block1)
    block1       = AveragePooling2D(pool_size=(1, 75//undersample), strides=(1, 15//undersample))(block1)
    block1       = Lambda(lambda x: K.log(K.clip(x, min_value = 1e-7, max_value = 10000)))(block1)
