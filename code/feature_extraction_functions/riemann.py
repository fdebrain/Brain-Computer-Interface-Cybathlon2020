...
class Riemann:
    def __init__(self, fs=250, n_classes=4, f_order=2, f_type='butter', f_min=4, f_max=40, bandwidths=[2, 4, 8, 16, 32]):
        ...
        self.f_type = f_type
        self.rho = 0.1
        self.f_bands = self.load_bands(
            bandwidths, f_min, f_max, f_order, f_type)
    ...
    def fit(self, X, y):
        ...
        self.C_ref_invsqrt = np.zeros(
            (len(self.f_bands), n_channels, n_channels))
        ...
        for band_idx, f_band in enumerate(self.f_bands):
            ...
            X_filt = filtering(X, fs=self.fs, f_order=self.f_order,
                               f_low=f_band[0], f_high=f_band[1], f_type=self.f_type)

            # Compute covariance matrices  (regularized version)
            cov_matrices = np.array([(1/(n_samples-1))*np.dot(X_t, X_t.T) + (
                self.rho/n_samples)*np.eye(n_channels) for X_t in X_filt])
    ...
    def transform(self, X):
        ...
        for band_idx, f_band in enumerate(self.f_bands):
            ...
            S_projections = np.array(
                [logm(c_ref_invsqrt @ cov_matrices[trial_idx] @ c_ref_invsqrt) for trial_idx in range(n_trials)])

            # Vectorize projected matrices
            S_projections_vec = np.array([half_vectorization(
                S_projections[trial_idx]) for trial_idx in range(n_trials)])

            feats = S_projections_vec if len(feats) == 0 else np.hstack([
                feats, S_projections_vec])
