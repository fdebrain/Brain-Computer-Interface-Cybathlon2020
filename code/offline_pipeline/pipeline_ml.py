...
class MLExperiment:
    ...
    def run(self):
        ...
        X_full, y, _, _, _, _ = EEGDataset(
            1, **self.dataloader_params).load_dataset()
        n_classes = len(np.unique(y))
        self.n_samples_tot = X_full.shape[-1]

        # Pre-processing - filtering
        if self.model_name == 'CSP':
            X_full = filtering(X_full, self.fs, f_order=5,
        ...
        for idx_train, idx_test in skf.split(X, y):
            ...
            if self.average_pred:
                ...
                for trial_idx in range(len(y_train)):
                    labels, cnts = np.unique(
                        y_pred_crops[trial2crops_train[trial_idx]], return_counts=True)
            ...
            if self.average_pred:
                ...
                for trial_idx in range(len(y_test)):
                    labels, cnts = np.unique(
                        y_pred_crops[trial2crops_test[trial_idx]], return_counts=True)
            ...
            self.train_score['accuracy'].append(
                accuracy_score(y_true_train, y_pred_train))
            self.train_score['kappa'].append(
                cohen_kappa_score(y_true_train, y_pred_train))
            self.test_score['accuracy'].append(
                accuracy_score(y_true_test, y_pred_test))
            self.test_score['kappa'].append(
                cohen_kappa_score(y_true_test, y_pred_test))
            self.conf_matrices['y_pred{}'.format(
                idx_kfold)] = y_pred_test if self.average_pred else y_pred_crops
        ...
        print('Mean training time per trial: {:.1f}ms'.format(
            np.mean(self.train_times)))
        print('Mean testing time per trial: {:.1f}ms'.format(
    ...
    def plot_results(self, get_tsne=False, full_data=False):
        ...
        fig = plot_full_barchart(self.test_score, n_pilots=self.n_folds,
                                 title='{} model - Competition Dataset - Session {}'.format(
                                     self.model_name, self.session_idx),
                                 fig=fig)
        ...
        if get_tsne:
            ...
            tsne_plot(X, y, title="t-SNE - Pilot 1 - Raw",
                      label_names=self.MI_labels, fig=fig, ax_idx=0)
    ...
    def logging(self):
        ...
        # Barchart
        fig = plot_full_barchart(self.test_score, n_pilots=self.n_folds,
                                 title='{} - Session {}'.format(self.model_name, self.session_idx))
        fig.get_axes()[0].set_xlabel('Fold')
